...
import { 
  FileText, Plus, Search, Edit3, Trash2, Eye, Send, CheckCircle,
  ChevronLeft, ChevronRight, X, Save, Clock, XCircle, ArrowRight,
  Calendar, Package, Printer, Download, Settings, Copy, AlertTriangle
} from 'lucide-react';
import { 
  Quote, QuoteItem, QuoteStatus, Customer, PriceCalculatorOrder,
  formatVND, formatDate, getQuoteStatusConfig 
} from '../../types/business';
...
const StatusBadge: React.FC<{ status: QuoteStatus }> = ({ status }) => {
  ...
  const Icon = {
    Clock, Send, CheckCircle, XCircle, AlertTriangle
  }[config.icon] || Clock;
...
const QuoteFormModal: React.FC<{
  ...
  initialItems?: { description: string; specifications: string; quantity: number; unitPrice: number; notes?: string }[];
  onSave: (data: {
    customerId: string;
...
}> = ({ isOpen, onClose, quote, customers, config, initialItems, onSave, onUpdate }) => {
  ...
  React.useEffect(() => {
    if (isOpen) {
      // Load draft orders from both old and new storage keys
      try {
...
// --- QUOTE PREVIEW MODAL ---
const QuotePreviewModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  quote: Quote | null;
...
// --- MAIN COMPONENT ---
export const QuotesPage: React.FC<QuotesPageProps> = ({ onClose }) => {
  const { 
  ...
  // Check for pending order from Price Calculator
  const [pendingOrderFromCalc, setPendingOrderFromCalc] = useState<any>(null);
  ...
  const [pendingOrderItems, setPendingOrderItems] = useState<any[] | null>(null);
  ...
  // Handlers
  const handleSave = async (data: Parameters<typeof addQuote>[0]) => {
    try {
      await addQuote(data);
    } catch (error) {
  ...
  return (
    <div className="h-full bg-slate-100 flex flex-col overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white p-6 shadow-lg shrink-0">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => { setEditingQuote(null); setIsEditorOpen(true); }}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-sm border p-4 mb-6">
            <div className="flex items-center gap-4">
              ...
              <select
                value={statusFilter}
                onChange={(e) => { setStatusFilter(e.target.value as QuoteStatus | 'all'); setCurrentPage(1); }}
                    ...
                    <tr key={quote.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setPreviewQuote(quote)} className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg" title="Xem">
                            <Eye size={16} />
                          </button>
                          <button onClick={() => { setEditingQuote(quote); setIsEditorOpen(true); }} className="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg" title="Sửa">
                            <Edit3 size={16} />
                          </button>
                          {quote.status === 'DRAFT' && (
                            <button onClick={() => updateQuoteStatus(quote.id, 'SENT')} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg" title="Đánh dấu đã gửi">
                              <Send size={16} />
                            </button>
                          )}
                          {['SENT', 'ACCEPTED'].includes(quote.status) && !quote.invoiceId && (
                            <button onClick={() => handleConvertToInvoice(quote)} className="p-2 text-green-500 hover:bg-green-50 rounded-lg" title="Tạo hóa đơn">
                              <ArrowRight size={16} />
                            </button>
                          )}
                          <button onClick={() => handleDelete(quote)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg" title="Xóa">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
            ...
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t bg-slate-50 flex items-center justify-between">
                <div className="text-sm text-slate-500">
                  Hiển thị {((currentPage - 1) * itemsPerPage) + 1} - {Math.min(currentPage * itemsPerPage, filteredQuotes.length)} / {filteredQuotes.length}
                </div>
                <div className="flex items-center gap-2">
      ...
      <QuoteFormModal
        isOpen={isEditorOpen}
        onClose={() => { setIsEditorOpen(false); setEditingQuote(null); setPendingOrderItems(null); }}
        quote={editingQuote}
        customers={customers}
        config={config?.quote || {}}
