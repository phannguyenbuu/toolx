...
import { CropEdgeDimInput } from './CropEdgeDimInput';
  ...
  return (
    <div className="flex-1 flex flex-col border-r border-slate-200 bg-slate-900/95 relative overflow-hidden">
      ...
      <div className="p-2 border-b border-slate-800 bg-slate-950/80 flex items-center justify-between gap-2 z-10 flex-wrap">
        ...
        {/* Transform controls: Khử trắng, Khôi phục gốc, Rotate, Flip, Grid, Reset */}
        <div className="flex items-center gap-1">
          {onOpenRemoveBgTab && (
            <button
              type="button"
              onClick={onOpenRemoveBgTab}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold transition border cursor-pointer select-none bg-indigo-950/90 hover:bg-indigo-900 border-indigo-700/80 text-indigo-300 hover:text-white shadow-xs"
              title="Mở tab Khử nền trắng (chỉnh slider ngưỡng sáng, bóng đổ, làm mềm viền)"
            >
          ...
          {hasOriginalBackup && onRestoreOriginal && (
            <button
              type="button"
              onClick={onRestoreOriginal}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold transition border cursor-pointer select-none bg-amber-950/80 hover:bg-amber-900 border-amber-700/80 text-amber-200 hover:text-white shadow-xs"
              title="Khôi phục lại ảnh gốc ban đầu (hoàn tác khử trắng / bù xén)"
            >
