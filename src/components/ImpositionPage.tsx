                  ...
                  {impositionHistory.length === 0 ? (
                    ...
                    <div className="space-y-2.5">
                      {impositionHistory.map((item) => (
                        <div
                          key={item.id}
                          className="p-3 bg-white hover:bg-slate-50/80 border border-slate-200 rounded-xl shadow-xs transition flex items-center justify-between gap-3 group"
                        >
                          ...
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-xs font-medium text-slate-800 truncate" title={item.title}>
                                  {item.title}
                                </span>
                              ...
                              <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-[11px] text-slate-500">
                                ...
                                {item.processMode && (
                                  <span className="capitalize px-1.5 py-0.2 bg-slate-100 rounded text-slate-600 text-[10px]">
                                    {item.processMode}
                                  </span>
                                )}
                          ...
                          <div className="flex items-center gap-1.5 flex-shrink-0">
                            {item.configSnapshot && (
                              <button
                                type="button"
                                onClick={() => restoreHistoryConfig(item)}
                                className="px-2.5 py-1.5 bg-violet-50 hover:bg-violet-100 text-violet-700 border border-violet-200 rounded-lg text-xs font-medium flex items-center gap-1 transition cursor-pointer"
                                title="Khôi phục lại khổ giấy và thông số bình trang này"
                              >
                                <RotateCcw size={13} />
                                <span>Nạp thông số</span>
                              </button>
                            )}
                            <button
                              type="button"
